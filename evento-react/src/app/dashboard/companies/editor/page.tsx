...
import AddCompanyModal from "@/components/popups/newCompany";
import CompaniesImportModal from "@/components/popups/companiesImport";
...
export default function App() {
	...
	const fetchCompanies = async () => {
		try {
			const response = await axiosInstanceAuth.get("/api/companies/editor");
	...
	const buttonsGroup = React.useCallback(() => {
		return (
			<>
				<AddCompanyModal action={fetchCompanies} />
				<CompaniesImportModal action={fetchCompanies} />
			</>
		)
	}, [fetchCompanies]);
	...
	return (
		<div>
			<h1 className="text-3xl mb-5">Мои компании</h1>
			<Table tableItems={companies} columns={columns} INITIAL_VISIBLE_COLUMNS={INITIAL_VISIBLE_COLUMNS} renderCell={renderCell} CustomAddComponent={buttonsGroup} onRowClick={(item) => { router.push(`/dashboard/companies/${item.id}`) }} />
		</div>
	);
}